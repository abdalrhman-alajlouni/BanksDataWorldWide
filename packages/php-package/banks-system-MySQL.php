	...
	class banksSQLSystem {
		...
		public function GetBankFromBIN($CardNoOrBIN) {
			...
			if ($response === false) {
				...
				$ResArray = [];
				$ResArray['status'] = "error";
				$ResArray['message'] = curl_error($curl);
				$ResArrayJSON = json_encode($ResArray, true);
				return $ResArrayJSON;	
				...
				if ($resultDiv) {
					// Find the table within the "result" div
					$tables = $resultDiv->getElementsByTagName('table');
					
					if ($tables->length > 0) {
						// Get the first table (assuming it's the one you want)
						$table = $tables->item(0);
						...
						foreach ($table->getElementsByTagName('tr') as $row) {
							...
							// Check if the header row has been found
							if (!$headerFound) {
								$header = $rowData;
								$headerFound = true;
								} else {
								...
								for ($i = 0; $i < count($header); $i++) {
									$rowDataAssoc[$header[$i]] = $rowData[$i];
								}
						...
						$ResArray = [];
						$ResArray['status'] = "error";
						$ResArray['message'] = 'No data found in the result.';
						$ResArrayJSON = json_encode($ResArray, true);
						return $ResArrayJSON;
					}
					} else {
